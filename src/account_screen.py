...
规则：
- 业务相关度：内容是否围绕客户专攻行业（橱柜/全铝/不锈钢/定制/装修避坑）
- 场景适配度：该账号的拍摄场景客户能否复现（工厂/展厅/客户家可复现；需要演员/特效/剧情的高分不了）
- 人设匹配度：出镜人设与客户（老板本人专业讲解型）的相似度
- 评分必须有区分度：不要都打 4 分——顶尖的给 5，明显弱的给 1~2 或建议排除，最多 3 个"深挖"
...
def write_md(out_md: Path, accounts: list):
    lines = [
        ...
        "",
        "| 昵称 | 搜索视频数 | 最高赞 | 主页作品数 | 主页爆款数 | 垂直 | 爆款 | 相关 | 场景 | 人设 | 建议 | 主要场景 | 推荐理由 | sec_uid |",
        "|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---|---|---|---|",
    ]
    for a in sorted(accounts, key=lambda x: -((x.get("业务相关度") or 0) * 10 + (x.get("场景适配度") or 0) + (x.get("爆款力") or 0))):
        lines.append(
            f"| {a['昵称']} | {a['视频数']} | {a['最高赞']} | {a.get('主页作品数','-')} | "
            f"{a.get('主页爆款数','-')} | {a.get('垂直度','-')} | "
            f"{a.get('爆款力','-')} | {a.get('业务相关度','-')} | {a.get('场景适配度','-')} | "
            f"{a.get('人设匹配度','-')} | {a.get('建议动作','-')} | {a.get('主要拍摄场景','')} | "
